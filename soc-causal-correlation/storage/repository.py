...
class AlertRepository:
    ...
    def create_alert(db: Session, alert_data: Dict[str, Any]) -> Alert:
        """
        Create a new alert record or return existing one if alert_id already exists
        ...
        Returns:
            Alert object (created or existing)
        """
        ...
        if existing_alert:
            # Update existing alert with new data (optional)
...
class IncidentRepository:
    ...
    def create_incident(db: Session, incident_data: Dict[str, Any]) -> Incident:
        """
        Create a new incident record or return existing one if incident_id already exists
        ...
        Returns:
            Incident object (created or existing)
        """
        ...
        if incident_id:
            existing_incident = db.query(Incident).filter(Incident.incident_id == incident_id).first()

        if existing_incident:
            # Update existing incident with new data (optional)
            for key, value in incident_data.items():
                if hasattr(existing_incident, key) and value is not None:
                    setattr(existing_incident, key, value)

            # Clear existing relationships and recreate them
            # Alerts
            existing_incident.alerts.clear()
            if alert_ids:
                alerts = db.query(Alert).filter(Alert.alert_id.in_(alert_ids)).all()
                existing_incident.alerts = alerts

            # Techniques
            existing_incident.techniques.clear()
            ...
            # Entities
            existing_incident.entities.clear()
